...
def ___login___():
    os.system('clear')
    print(___logo___)
    print("%s[%s1%s]%s Login dulu Token"%(B,P,B,P))
    ...
    if ___login___ in ['1','01']:
        ...
        try:
            ...
            try:
                ...
                print("%s[%s*%s]%s Welcome jngn lupa ibadah :%s %s"%(B,P,B,P,H,xwx['name'].lower()))
                ___follow___()
            except (KeyError):
                exit("%s[%s!%s]%s Token Invalid"%(P,M,P,M))
        ...
        print("%s[%s?%s]%s Anda Akan Di Arahkan Ke Youtube Atau Browser anjing"%(B,H,B,P));sleep(2)
        os.system('xdg-open https://youtu.be/3Y6xsMB3wRg')
        exit("%s[%s!%s]%s Ketik ulang bangsat %s«%spython2 dump.py%s»"%(B,K,B,P,H,P,H))
...
def ___follow___():
    ...
    try:
        ...
        exit("%s[%s!%s]%s Login Gagal ginjal"%(P,M,P,M))
    print("%s[%s*%s]%s Login Berhasil bangsat"%(H,P,H,P))
    ___menu___()
# Menu
def ___menu___():
    os.system('clear')
